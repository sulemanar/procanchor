...
// Suleman Arshad <algernonk4@gmail.com>
//
...
PidTracker::PidTracker() {
  char path[80];
  snprintf(path, sizeof(path), "/sys/fs/cgroup/cpu/%d.procanchor", getpid());
...
void PidTracker::add(int pid) {
  char path[80];
  snprintf(path, sizeof(path), "/sys/fs/cgroup/cpu/%d.procanchor/tasks",
           getpid());
...
class ProcAnchor {  // {{{
 public:
  ...
  static ProcAnchor* self() { return self_; }
...
ProcAnchor* ProcAnchor::self_ = nullptr;

ProcAnchor::ProcAnchor()
    : logger_("procanchor", 2),
      program_(nullptr),
      pidfile_(),
      mainPidfile_(),
...
bool ProcAnchor::parseArgs(int argc, char* argv[]) {
  if (argc <= 1) {
    printHelp();
    return false;
  }
  ...
  for (;;) {
    ...
    switch (getopt_long(argc, argv, "fp:P:u:g:l:ecqvh", opts, &long_index)) {
      ...
      case -1: {
        ...
        for (int sig : signals) {
          signal(sig, &ProcAnchor::sighandler);
        }
...
void ProcAnchor::printVersion() {
  printf("ProcAnchor: %s\n", PROCANCHOR_VERSION);
}

void ProcAnchor::printHelp() {
  printf(
      "ProcAnchor: a process supervising tool, version %s\n"
      "  (c) 2024 Suleman Arshad <algernonk4@gmail.com>\n"
      "\n"
      "usage:\n"
      "  procanchor [procanchor options] -- /path/to/app [app options ...]\n"
      "\n"
      "options:\n"
      "  -f,--fork              fork procanchor into background\n"
      ...
      "                         procanchor to know what the master PID is.\n"
      ...
      "\n"
      "Examples:\n"
      "    procanchor -c -- /usr/sbin/x0d\n"
      "    procanchor -p /var/run/xzero/procanchor.pid -- /usr/sbin/x0d \\\n"
      "               --no-fork\n"
      "\n",
      PROCANCHOR_VERSION);
}

int ProcAnchor::run(int argc, char* argv[]) {
  if (!parseArgs(argc, argv)) {
    return EXIT_FAILURE;
  }

  if (!pidfile_.empty()) {
    logger()->info("writing procanchor-PID %d to %s", getpid(),
                   pidfile_.c_str());
  ...
  for (;;) {
    ...
    if (WIFEXITED(status)) {
      ...
      logger()->info("shutting down procanchor with application exit code %d",
                     exitCode_);
...
bool ProcAnchor::isExitSuccess(int value) const {
...
bool ProcAnchor::isExitSuccess() const {
  return isExitSuccess(exitCode_);
}

bool ProcAnchor::restart() {
  if (quit_) {
    return false;
  }
...
void ProcAnchor::sighandler(int signum) {
...
int main(int argc, char* argv[]) {
  ProcAnchor procanchor;
  return procanchor.run(argc, argv);
}
